...
function CasaDetalhesModal({
  casaId,
  onClose,
  onAbrirReservaModal,
  onContatarLocadorModal,
}) {
  ...
  const fetchCasaDetalhes = useCallback(async () => {
    ...
    try {
      ...
      if (data.imagens && data.imagens.length > 0) {
        ...
        setImagemPrincipal(
          `https://placehold.co/600x400/A8D5E2/333?text=${encodeURIComponent(
            data.endereco?.substring(0, 20) || "Casa"
          )}`
        );
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className={styles.modalOverlay} onClick={handleOverlayClick}>
        ...
        >
          <div className={styles.loadingStateModal}>
            Carregando detalhes da casa...
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="casaModalTitle"
    >
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalBody}>
                ...
                {casa.imagens.map((img) => (
                  <div
                    ...
                    className={`${styles.thumbnailItem} ${
                      img.url === imagemPrincipal ? styles.thumbnailAtiva : ""
                    }`}
                    onClick={() => handleSelecionarImagem(img.url)}
                    onKeyPress={(e) =>
                      e.key === "Enter" && handleSelecionarImagem(img.url)
                    }
                    tabIndex={0}
                    role="button"
                  ...
                  >
                    <img
                      src={img.url}
                      alt={`Thumbnail de ${casa.endereco} ${
                        casa.imagens.indexOf(img) + 1
                      }`}
                    />
                  </div>
                ))}
          ...
          <div className={styles.infoContainer}>
            <h2 id="casaModalTitle" className={styles.casaTitulo}>
              {casa.endereco}, {casa.numero}
            </h2>
            <p className={styles.casaLocalizacao}>
              <LocalIcon /> {casa.cidade}
              {casa.estado ? `, ${casa.estado}` : ""}{" "}
              {casa.cep ? `- CEP: ${casa.cep}` : ""}
            </p>

            {typeof casa.precoPorNoite === "number" && (
              <p className={styles.casaPreco}>
                <PrecoIcon /> R${" "}
                {casa.precoPorNoite.toFixed(2).replace(".", ",")} / noite
              </p>
            )}
            ...
            <div className={styles.detalhesGrid}>
              ...
              {casa.complemento && (
                <div className={styles.detalheItem}>
                  <h4>
                    <span className={styles.detailIcon}>ℹ️</span> Complemento
                  </h4>
              ...
              {casa.locador && (
                <div className={styles.detalheItem}>
                  <h4>
                    <UserIcon /> Sobre o Locador
                  </h4>
                  <p>
                    <strong>Nome:</strong> {casa.locador.name}
                  </p>
                  <p>
                    <strong>Email:</strong> {casa.locador.email}
                  </p>
                </div>
              )}
            </div>
              ...
              <div className={styles.acoesContainerModal}>
                <button
                  ...
                  title={
                    casa.precoPorNoite === null ||
                    casa.precoPorNoite === undefined
                      ? "Reserva indisponível (sem preço)"
            ...
            {!isAuthenticated && casaId && (
              <div className={styles.loginPromptModal}>
                <p>
                  Faça{" "}
                  <Link to={`/login?redirect=/casas/${casaId}`}>login</Link> ou{" "}
                  <Link to={`/register?redirect=/casas/${casaId}`}>
                    cadastre-se
                  </Link>{" "}
                  para reservar ou contatar o locador.
                </p>
              </div>
            )}
          </div>
