...
function ReservaModal({ isOpen, onClose, casa, onReservaSucesso }) {
  ...
  const fetchDetalhesCasaParaDisponibilidade = useCallback(async () => {
    ...
    setError("");
    try {
      const response = await fetch(`http://localhost:5000/api/casa/${casa.id}`);
      if (!response.ok) {
        const errText = await response.text();
        console.error("Resposta de erro do backend (disponibilidade):", errText);
      ...
      setError("Não foi possível verificar a disponibilidade da casa no momento.");
      setPeriodosIndisponiveis([]);
    } finally {
  ...
  const handleSubmitReserva = async (e) => {
    ...
    const fimSolicitadoMenosUmDiaStr = paraDataUTCString(new Date(checkOutSelecionado.getTime() - (24 * 60 * 60 * 1000)));

    for (const periodo of periodosIndisponiveis) {
      const inicioIndisponivel = periodo.inicio;
      const fimIndisponivel = periodo.fim;

      if (inicioSolicitadoStr <= fimIndisponivel && fimSolicitadoMenosUmDiaStr >= inicioIndisponivel) {
         const tipoConflito = periodo.tipo === 'reserva' ? 'reservado' : 'bloqueado pelo locador';
         setError(`Conflito de datas: O período de ${new Date(inicioIndisponivel + 'T00:00:00Z').toLocaleDateString('pt-BR', {timeZone:'UTC'})} a ${new Date(fimIndisponivel + 'T00:00:00Z').toLocaleDateString('pt-BR', {timeZone:'UTC'})} está ${tipoConflito}.`);
         setIsLoading(false);
         return;
      }
    }
    ...
    try {
      ...
      toast.success("Solicitação de reserva enviada! Você será redirecionado para a simulação de pagamento.");
      onClose();
  ...
  return (
    <div className={styles.modalOverlay} onClick={handleOverlayClick}>
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        <button className={styles.closeButton} onClick={onClose} aria-label="Fechar modal de reserva">
          &times;
        </button>
        ...
        {loadingDisponibilidade && <p className={styles.loadingMessage}>Verificando disponibilidade...</p>}
        ...
        <form onSubmit={handleSubmitReserva} className={styles.reservaForm}>
          <div className={styles.formGroup}>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (dataCheckOut && new Date(e.target.value) >= new Date(dataCheckOut)) {
                    setDataCheckOut("");
                }
              }}
          ...
          <div className={styles.formGroup}>
            ...
            <input
              ...
              min={dataCheckIn ? getMinDateParaInput(dataCheckIn) : formatarDataParaAPI(new Date())}
              required
          ...
          {casa.precoPorNoite > 0 && valorCalculado > 0 && dataCheckIn && dataCheckOut && (new Date(dataCheckOut) > new Date(dataCheckIn)) && (
            <div className={styles.valorTotalInfo}>
              <p>
                ...
                (
                {Math.ceil(
                  Math.abs(new Date(dataCheckOut + 'T00:00:00') - new Date(dataCheckIn + 'T00:00:00')) /
                    (1000 * 60 * 60 * 24)
                )}{" "}
                noites)
              </p>
          ...
          <div className={styles.formActions}>
            ...
            <button
              ...
              disabled={isLoading || loadingDisponibilidade}
            >
              {isLoading ? "Enviando..." : loadingDisponibilidade ? "Verificando..." : "Solicitar Reserva"}
            </button>
          </div>
        </form>
      </div>
