...
function ListaConversas({
  ...
  const handleExcluirConversa = async (idConversa, e) => {
    ...
    if (
      !window.confirm(
        "Tem certeza que deseja excluir esta conversa? Esta ação não pode ser desfeita e a conversa será removida para ambos os participantes."
      )
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `https://apiunihosp.onrender.com/api/conversas/${idConversa}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <ul className={styles.listaConversas}>
      ...
      {error && conversas.length > 0 && (
        <p className={styles.errorInline}>Erro ao atualizar: {error}</p>
      )}
      {conversas.map((conversa) => {
        const outroParticipante =
          userType === "hospede" ? conversa.locador : conversa.hospede;
        ...
        if (ultimaMensagem) {
          if (
            (userType === "hospede" &&
              ultimaMensagem.remetenteHospedeId === userId) ||
            (userType === "locador" &&
              ultimaMensagem.remetenteLocadorId === userId)
          ) {
        ...
        return (
          <li
            ...
            className={`${styles.itemConversa} ${
              conversa.id === conversaAtivaId ? styles.ativa : ""
            } ${conversa.contagemNaoLidas > 0 ? styles.naoLida : ""}`}
            onClick={() =>
              !estaDeletando &&
              onConversaSelecionada(conversa.id, outroParticipante)
            }
            tabIndex={0}
            onKeyPress={(e) =>
              !estaDeletando &&
          ...
          >
            ...
            <div className={styles.infoConversa}>
              <div className={styles.nomeEData}>
                <span className={styles.nomeParticipante}>
                  {outroParticipante?.name || "Usuário Desconhecido"}
                </span>
                {ultimaMensagem && (
              ...
              <p className={styles.previewMensagem}>
                {conversa.contagemNaoLidas > 0 && !remetenteUltimaMensagem ? (
                  <strong>{previewMensagem}</strong>
                ) : (
                  <>
            ...
            <div className={styles.acoesItemConversa}>
              {conversa.contagemNaoLidas > 0 && (
                <span className={styles.badgeNaoLidasItem}>
                  {conversa.contagemNaoLidas}
                </span>
              )}
              <button
