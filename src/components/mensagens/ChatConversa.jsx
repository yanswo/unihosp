...
function ChatConversa({
  ...
  const [infoOutroUsuario, setInfoOutroUsuario] = useState(
    infoOutroUsuarioProp || null
  );
  ...
  const marcarMensagensComoLidas = useCallback(async () => {
    ...
    try {
      await fetch(
        `https://apiunihosp.onrender.com/api/conversas/${conversaId}/marcar-como-lidas`,
        {
          method: "PUT",
          headers: {
  ...
  const fetchMensagens = useCallback(async () => {
    ...
    try {
      const responseMensagens = await fetch(
        `https://apiunihosp.onrender.com/api/conversas/${conversaId}/mensagens`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      if (!responseMensagens.ok) {
        const errData = await responseMensagens
          .json()
          .catch(() => ({ error: `Erro HTTP ${responseMensagens.status}` }));
      ...
      if (!infoOutroUsuario) {
        console.warn(
          "ChatConversa: infoOutroUsuario não foi fornecida via prop. Header do chat pode ficar incompleto."
        );
      }
    } catch (err) {
    ...
    <div className={styles.chatContainer}>
      <header className={styles.chatHeader}>
        <button
          ...
          className={styles.botaoVoltarMobile}
        >
          <VoltarIcon />
        </button>
        ...
        <h3 className={styles.chatComNome}>
          {infoOutroUsuario?.name || "Carregando..."}
        </h3>
      </header>
      ...
      <div className={styles.mensagensLista}>
        {mensagens.map((msg) => {
          ...
          return (
            <div
              key={msg.id}
              className={`${styles.mensagemItem} ${
                ehMinhaMensagem ? styles.minhaMensagem : styles.outraMensagem
              }`}
            >
      ...
      {error && enviando && (
        <p className={styles.errorEnvioMensagem}>{error}</p>
      )}
      {!error && enviando && (
        <p className={styles.enviandoStatus}>Enviando...</p>
      )}
