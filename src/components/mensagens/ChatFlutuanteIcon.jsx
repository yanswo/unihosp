...
function ChatFlutuanteIcon() {
  ...
  const fetchTotalNaoLidas = useCallback(async () => {
    if (!isAuthenticated || !token || (userType !== 'hospede' && userType !== 'locador')) {
      setTotalNaoLidas(0);
      return;
    }
    try {
      const response = await fetch('http://localhost:5000/api/mensagens/nao-lidas/total', {
        headers: {
      ...
      if (response.ok) {
        ...
        console.error("Erro ao buscar total de mensagens não lidas:", response.status);
        setTotalNaoLidas(0);
      }
    } catch (error) {
  ...
  return (
    <button
      ...
      title={`Abrir Minhas Mensagens ${totalNaoLidas > 0 ? `(${totalNaoLidas} não lidas)` : ''}`}
      aria-label={`Abrir Minhas Mensagens ${totalNaoLidas > 0 ? `(${totalNaoLidas} não lidas)` : ''}`}
    >
      <ChatIcon />
      {totalNaoLidas > 0 && (
        <span className={styles.notificationBadge}>{totalNaoLidas > 9 ? '9+' : totalNaoLidas}</span>
      )}
    </button>
  );
}
