...
function MinhasReservas() {
  ...
  const handleContatarLocadorDoModal = async (locadorId, casaIdContexto) => {
    ...
    try {
      ...
      console.error("Erro ao iniciar conversa do modal de detalhes (Minhas Reservas):", err);
  ...
  if (error && reservas.length === 0) return <div className={styles.error}>{error}</div>;

  return (
    <>
      {error && reservas.length > 0 && <p className={`${styles.error} ${styles.errorInline}`}>{error}</p>}
        ...
        <ul className={styles.reservasList}> 
          {reservas.map((reserva) => {
            ...
            return (
              <li
                key={reserva.id}
                className={`${styles.reservaCard} ${styles.reservaCardLayoutRow} ${ // Aplicando layout em linha e status
                  ...
                  <p>
                    ...
                    <span
                      className={`${styles.status} ${
                        styles["status" + reserva.status?.replace(/\s+/g, "")] || "" // Para cores de status
                      }`}
                    >
                      {reserva.status || "N/A"}
                    </span>
