...
function HistoricoReservasHospede() {
  ...
  const fetchHistorico = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `https://apiunihosp.onrender.com/api/hospede/reservas/historico`,
        {
  ...
  const handleContatarLocadorDoModal = async (locadorId, casaIdContexto) => {
    ...
    try {
      ...
      console.error(
        "Erro ao iniciar conversa do modal de detalhes (Histórico):",
        err
      );
          ...
          {historico.map((reserva) => {
            ...
            const altDaImagem =
              reserva.casa?.imagens && reserva.casa.imagens.length > 0
                ...
                : `Sem foto para ${
                    reserva.casa?.endereco || "Casa do Histórico"
                  }`;
