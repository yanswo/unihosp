...
function MeusFavoritos() {
  ...
  const fetchFavoritos = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `https://apiunihosp.onrender.com/api/hospede/favoritos`,
        {
      ...
      if (Array.isArray(data)) {
        ...
        setFavoritos([]);
        console.warn(
          "MeusFavoritos: Formato de dados de favoritos inesperado.",
          data
        );
      }
    } catch (err) {
  ...
  const handleContatarLocadorDoModal = async (locadorId, casaIdContexto) => {
    ...
    try {
      ...
      console.error(
        "Erro ao iniciar conversa do modal de detalhes (Favoritos):",
        err
      );
        ...
        <div className={styles.favoritosGrid}>
          {favoritos.map((favorito) => {
            const imagemDaCasaUrl =
              favorito.casa?.imagens && favorito.casa.imagens.length > 0
                ...
                <div className={styles.favoritoInfo}>
                  ...
                  {typeof favorito.casa?.precoPorNoite === "number" && (
                    <p className={styles.precoPorNoite}>
                      R${" "}
                      {favorito.casa.precoPorNoite.toFixed(2).replace(".", ",")}{" "}
                      / noite
                    </p>
                  )}
