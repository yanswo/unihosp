...
function CasaDetalhesPage() {
  ...
  const handleContatarLocadorAqui = async () => {
    ...
    try {
      ...
      console.error(
        "Erro ao iniciar conversa a partir dos detalhes da casa:",
        err
      );
    ...
    <div className={styles.pageContainer}>
      ...
      <main className={styles.detalhesContent}>
        <div className={styles.imageGallery}>
          ...
          {(casa.imagens?.length === 0 || !casa.imagens) && casa.endereco && (
            <div className={styles.imagemPrincipalContainer}>
              ...
              {}
              <img
                src={`https://placehold.co/600x400/A8D5E2/333?text=${encodeURIComponent(
                  casa.endereco?.substring(0, 20) || "Casa"
                )}`}
        ...
        <div className={styles.infoContainer}>
          <h1 className={styles.casaTitulo}>
          ...
          {typeof casa.precoPorNoite === "number" && (
            <p className={styles.casaPreco}>
              <PrecoIcon /> R$ {casa.precoPorNoite.toFixed(2).replace(".", ",")}{" "}
              / noite
            </p>
          )}
          ...
          {!isAuthenticated && (
            <div className={styles.loginPrompt}>
              <p>
                Faça <Link to={`/login?redirect=/casas/${casaId}`}>login</Link>{" "}
                ou{" "}
