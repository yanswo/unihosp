...
function PaginaMensagens() {
  ...
  const [refreshConversasListKey, setRefreshConversasListKey] = useState(Date.now());

  const fetchDetalhesConversaParaChat = useCallback(async (idConversa) => {
    if (!token || !idConversa) return;
    setLoadingInfoConversa(true);
    try {
      const response = await fetch(`http://localhost:5000/api/conversas/detalhes/${idConversa}`, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
      if (!response.ok) {
        const errData = await response.json().catch(() => ({error: "Erro ao buscar dados da conversa"}));
      ...
      console.error("Erro ao buscar detalhes da conversa para o chat:", error);
      setInfoOutroUsuarioChat(null);
      navigate('/mensagens');
  ...
  }, [conversaIdDaUrl, fetchDetalhesConversaParaChat, conversaSelecionadaId, infoOutroUsuarioChat]);


  const handleSelecionarConversa = useCallback((idConversa, outroUsuarioInfo) => {
  ...
  }, [navigate, conversaIdDaUrl, infoOutroUsuarioChat, fetchDetalhesConversaParaChat]);
  ...
  const handleConversaExcluidaDaLista = useCallback((idConversaExcluida) => {
    setRefreshConversasListKey(Date.now());
  ...
  return (
    <div className={styles.paginaMensagensContainer}>
      ...
      <div className={styles.conteudoPrincipalMensagens}>
        <div 
          className={`${styles.listaConversasContainer} ${conversaSelecionadaId ? styles.esconderEmMobile : ''}`}
        >
        ...
        <div 
          className={`${styles.chatConversaContainer} ${!conversaSelecionadaId ? styles.esconderEmMobile : ''}`}
        >
          {conversaSelecionadaId ? (
            loadingInfoConversa && !infoOutroUsuarioChat ?
            <div className={styles.placeholderChat}><p>Carregando dados da conversa...</p></div> 
            :
            <ChatConversa 
            ...
            <div className={styles.placeholderChat}>
              ...
              <p className={styles.dicaPlaceholder}>Ou inicie uma nova conversa na página de uma casa.</p>
            </div>
          )}
        </div>
      </div>
